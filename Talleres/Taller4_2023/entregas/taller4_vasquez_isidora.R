...
#A lo lago del taller, se modificaron los dataframe, para que nos diera altiro las coordenadas en (long, lat)
...
p2d <-st_point(c(longitud,latitud))
##Punto de 3 dimensiones
p3d <-st_point(c(longitud,latitud,elevacion))
##Punto de 4 dimensiones
p4d <-st_point(c(longitud,latitud,elevacion,direccion))

##Revision de la estructura del objs creados
str(p2d)
str(p3d)
str(p4d)
...
multipoint <- st_multipoint(dfmatrix)
linestring <- st_linestring(dfmatrix)
...
matrizdf2<-as.matrix(df2[,2:3])

multilinestring<-st_multilinestring(
  list(matrizdf2[1:4,],matrizdf2[5:8,]))
...
matrizdf_pol<-as.matrix(df_pol)

polygon<-st_polygon(
  list(
    rbind(matrizdf_pol,matrizdf_pol[1,])
  )
)
...
#5.Utilice las coordenadas en el data.frame df_mpol para crear un objeto sfg de tipo MULTIPOLYGON que contenga dos polygonos. Verifique que la clase y geometría del objeto es la correspondiente y haga un plot de cada objeto.
...
#capa1
area_preservacion_ecologica<-st_read("Biodiversidad/BIO_area_preservacion")
st_geometry_type(area_preservacion_ecologica)
str(area_preservacion_ecologica)
st_crs(area_preservacion_ecologica)

#capa2
 area_prohibicion_caza<-st_read("Biodiversidad/BIO_area_prohibicion_caza")
...
st_write(snaspe_RM2,dsn="areas_protegidad_RM.gpkg",layer="areas_protegidad_RM.gpkg")
...
st_write(Emisiones_fuentes_puntuales_DIOX2,dsn="fuentes_pun_cont_RM_CO2.gpkg",layer="fuentes_pun_cont_RM_CO2.gpkg")
