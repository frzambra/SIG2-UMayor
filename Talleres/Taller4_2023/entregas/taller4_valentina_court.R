...
p1<-st_point(c(lon,lat))
str(p1)
p2<-st_point(c(lon, lat,elevacion))
str(p2)
p3<-st_point(c(lon, lat, elevacion, direccion))
str(p3)
...
dfml <- st_multilinestring(list(dfm[1:4,2:1],dfm[5:8,2:1]))
plot(dfml)
