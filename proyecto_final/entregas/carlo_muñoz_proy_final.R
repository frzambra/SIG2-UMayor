...
#Temperatura
files<-list.files("C:/Users/Carlo/Desktop/trabajo.s/era5-monthly_2m_mean_temperature", pattern = "tif$", full.names = TRUE)
files
temp<-rast(files)
plot(temp)

#Precipitacion
files2<-list.files("C:/Users/Carlo/Desktop/trabajo.s/era5-monthly_total_precipitation",pattern = "tif$",full.names = TRUE)
files2
...
library(sf)
#Excel
dta<-read.csv("C:/Users/Carlo/Desktop/trabajo.s/planilla_presencia_frickius_variolosus.csv", sep=";")
str(dta)
...
saveRDS(df_final,"C:/Users/Carlo/Desktop/trabajo.s/datos_predictores.rds")
...
tmap_mode("plot")
#con tmap
tm_shape(temp_prom_anual) +tm_raster() tm_dots()+ tm_shape(data_unida_t)tm_shape(temp_prom_anual)+ tm_dots()

tm_shape(data_unida_t)+ tm_dots(col="presencia",size = .2)