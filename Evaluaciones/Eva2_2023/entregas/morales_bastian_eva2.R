...
#R puede utilizarse como un Sistema de Información Geográfica (SIG) mediante la implementación de paquetes específicos diseñados para el manejo y análisis de datos geoespaciales.También es posible conectar R con bases de datos espaciales y construir aplicaciones web interactivas que incluyan mapas y análisis geoespaciales. Dos paquetes clave son sp y sf
...
#En R, se utilizan principalmente dos tipos de objetos para trabajar con datos geoespaciales
#Vectores (sf): En R, los datos geoespaciales vectoriales se representan mediante objetos de clases como SpatialPoints, SpatialLines, SpatialPolygons (del paquete sp) o sf (Simple Features), que pueden contener información sobre ubicaciones puntuales, líneas o polígonos respectivamente, con atributos asociados a cada geometría
#Rasters (raster): Para datos geoespaciales tipo raster (cuadriculados), se utilizan objetos de clase RasterLayer, RasterStack, RasterBrick (del paquete raster) o de clase stars (del paquete stars). Estos objetos representan datos distribuidos en una cuadrícula regular, como imágenes satelitales o mapas de cobertura.
...
# Geometrías geométricas (GEOMETRY)
...
#d. (15pts) guarde el objeto calag_rom_buf en formato geopackage y con el nombrecalidad_agua_romeral_buffer_1000m
 ...
 #RASTER
 #1. (30pts) Cree un raster de 4 filas y 4 columnas que se encuentre entre 35°20’ y 37°30’ de latitudSur; y 71°50’ y 70°15’ de longitud Oeste.
...
nuevor2 <- r2
nuevor2[nuevor2 < 46 | nuevor2 > 54] <- NA
plot(nuevor2)
