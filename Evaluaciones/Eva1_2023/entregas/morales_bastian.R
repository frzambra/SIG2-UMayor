...
promedio2 <- sum(exvec2) / length(exvec2)
promedio2
...
rnorm(6) + rnorm(2)
?rnorm
# Densidad, función de distribución, función cuantil y generación aleatoria para la distribución normal con media igual a media y desviación estándar igual a sd
# La operación suma dos distribuciones normales. Especificamente suma una distribucion normal aleatoria con 6 observaciones y una distribucion normal aleatoria con 2 observaciones.

#6 (10pts) Cuáles son los tipos de vectores en R, de un ejemplo de cada caso.

#Numericos
num_vector <- c(1, 3, 5, 7)
class(num_vector)
#Enteros
ent_vector <- c(1L, 2L, 3L, 4L)
class(ent_vector)

#Character
char_vector <- c("bastian", "isidora", "alonso")
class(char_vector)
...
#Factor 
fac_vector <- c('a','a','b','c','c','c')
v <- factor(fac_vector,levels = c('a','b','c')) 
class(v)
...
## diferencia entre los dos códigos es que el primer código simplifica automáticamente el resultado a un vector, mientras que el segundo código mantiene el resultado como una matriz de una sola columna debido al uso de drop = FALSE.
...
v1 <- c(1, 2, 3, 4, 5)
v2 <- c("Alonso", "Isidora", "Bastian", "Francisco", "Javier")
v3 <- c(TRUE, FALSE, TRUE, TRUE, FALSE)
df <- data.frame(Numeros=v1,Character=v2,Logical=v3)
df
...
##objeto de clase data.frame y uno de clase sf es que el primero se utiliza para almacenar datos tabulares sin información geoespacial, mientras que el segundo se utiliza para almacenar datos geoespaciales que incluyen información sobre la ubicación espacial y permite realizar análisis espaciales.
...
data <- read.csv('data/datos_ema_203.csv')
...
ph <- mean(data$humed_rel_promedio, na.rm = TRUE)
ph
mp <- data[data$humed_rel_promedio > ph, ]
mp


#(10pts) Con el set de datos resultado anterior, seleccione las variables de fecha_hora, humed_rel_promedio, veloc_max_viento.
  
vs <- mp[, c("fecha_hora", "humed_rel_promedio", "veloc_max_viento")]
vs 
...
write.csv2(vs,"datos_21.csv")
read.csv2("datos_21.csv")
