...
data_info <- read.csv2('data/estaciones.csv') #configuracion latina
...
data_info[47,] #para seleccionar una estacion
...
head(data) #mostrar la primeras 6 filas de un data frame
tail(data) #mostrar la ultimas 6 filas de un data frame
dim(data) #numero tanto de filas como de columna 
nrow(data) #numero de filas
ncol(data) #numereo de columnas

#6 (2pt) Haga un resumen estadístico de todas las variables que contiene data.

summary(data) #funcion que  entrega un resumen estadistico de cada variable del data.frame
...
fecha <- substr(x=data_temp$fecha,start = 1,stop = 7) #solo deja fecha y hora
...
min(data_temp_enero$temp_max, na.rm = TRUE)
mean(data_temp_enero$temp_max, na.rm = TRUE) # na.rm para no considerar NA
