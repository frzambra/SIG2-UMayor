...
#FORMA1
id<-32
r<-data_info$ema==id #para obtener la fila del ema
r
data_info[r,]
#FOMRA2 (mas de una estacion) (id<-c(25,50))
...
dim(data)#(numero de observacon y varibles que son la socmulmnas)
class(data)#(clase dle objeto)
class(data$station_id)
class(data$fecha_hora)
class(data$temp_promedio_aire)
class(data$precipitacion_horaria)
#un resumen de la estrucutra con str (forma de sacare rapido la clase de las variables)
str(data)
#funciones para inspeccionar
head(data)#extrae las primeras 6 primeras filas
tail(data)#extrae las 6 ultimas filas
dim(data)
nrow(data)
ncol(data)
...
data_temp<-data.frame(fecha.hora,temp_air,temp_min,temp_max)
data_temp
#forma 2
...
enero<-"2021-01"
n<-data_temp_2$fecha==enero
data_temp_enero<-data_temp_2[n,]
summary(data_temp_enero)

View(data_temp_enero)
#forma 2, un poco mas corta
fecha<-substr(x=data_temp$fecha_hora,start = 1,stop=7)
